...
* 문제1<br>
    * 문제점 : 도서 정보를 수집하여 python 리스트에 삽입하였지만 DB에 저장되지 않는 문제
    * 해결방안 : 주소에 들어가는 값을 특정 변수로 새로 지정하였기 때문에 포매팅을 적용하여 문제 해결
 
* 문제2<br>
    * 문제점 : csv 파일의 데이터를 mysql 프로그램에 import를 시도했지만 되지 않는 문제
    * 해결방안 : 테이블의 컬럼이 될 csv 파일의 첫 번째 줄의 글자를 한글로 변환했더니 문제 해결
